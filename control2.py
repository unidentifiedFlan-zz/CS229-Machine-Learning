...
prob_sa = np.full((NUM_STATES, NUM_ACTIONS, NUM_STATES), init_prob)
#Value of state
value_func = np.random.uniform(0.0, 0.1, NUM_STATES)
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # model.
    ###### BEGIN YOUR CODE ######
    EVsa = np.sum(prob_sa*value_func, axis=2)
    ...
    if new_state == NUM_STATES - 1:
        ...
        while max_change > TOLERANCE:
            ...
            new_Vs = rewards + GAMMA*np.max(np.sum(prob_sa*value_func, axis=2), axis=1)
            change = np.abs(new_Vs - value_func)
