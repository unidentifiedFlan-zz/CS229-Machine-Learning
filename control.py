...
###### BEGIN YOUR CODE ######
init_prob = 1/NUM_STATES

#Transition probability
Psas = np.full((NUM_STATES, NUM_ACTIONS, NUM_STATES), init_prob)
#Value of state
Vs = np.random.uniform(0.0, 0.1, NUM_STATES)
#Reward count for s'
Rs = np.zeros(NUM_STATES)
#Expected value taking action a in state s
EVsa = np.zeros((NUM_STATES, NUM_ACTIONS))

#Counter arrays:
#Number of times s transitioned to s' via action a
NTsas = np.zeros((NUM_STATES, NUM_ACTIONS, NUM_STATES))
#Frequency of state s'
Fs = np.zeros(NUM_STATES)
#Cumulative reward
CRs = np.zeros(NUM_STATES)
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # model.
    ###### BEGIN YOUR CODE ######
    EVsa = np.sum(Psas*Vs, axis=2)
    action = np.argmax(EVsa[state, :])
    ###### END YOUR CODE ######
    ...
    NTsas[state, action, new_state] += 1
    CRs[new_state] += R
    Fs[new_state] += 1
    ...
    if new_state == NUM_STATES - 1:
        ...
        ###### BEGIN YOUR CODE ######
        #Fs[Fs == 0] = 1
        #Psas = NTsas / Fs[None, None, :]
        #Psas[Psas == 0] = init_prob

        total = NTsas.sum(axis=2)
        rTotal = []
        for i in range(NUM_STATES):
            rTotal.append(total.T)
        rTotal = np.asarray(rTotal).T
        mask = NTsas > 0
        Psas[mask] = NTsas[mask] / rTotal[mask]

        mask = Fs > 0
        Rs[mask] = CRs[mask]/Fs[mask]
        ...
        while max_change > TOLERANCE:
            new_Vs = np.zeros(NUM_STATES)
            #for s in range(NUM_STATES):
                #EVsa[s, :] = np.sum(np.dot(Psas[s, :, :], Vs))
            new_Vs = Rs + GAMMA*np.max(np.sum(Psas*Vs, axis=2), axis=1)
            change = np.abs(new_Vs - Vs)
